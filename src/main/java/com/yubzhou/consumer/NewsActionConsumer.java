...
public class NewsActionConsumer {
	...
	public void syncToMySQL() {
		...
		// 分批更新到数据库，优化批次大小
		int batchSize = 500;
		for (int from = 0; from < newsList.size(); from += batchSize) {
			int to = Math.min(from + batchSize, newsList.size());
			List<News> batch = newsList.subList(from, to);
			if (!batch.isEmpty()) {
				newsService.updateBatchById(batch);
			}
		}
	}
